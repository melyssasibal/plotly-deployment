...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sampleValuesSorted = sampleValues.sort((a,b) => a - b).reverse();
    var sampleValuesTopTen = sampleValuesSorted.slice(0, 10);
    var yticks = otuIds.map(x => `OTU ${x}`).slice(0,10).reverse();
    
    // 8. Create the trace for the bar chart. 
    var barData = [
      {
        x: sampleValuesTopTen.reverse(),
        y: yticks,
